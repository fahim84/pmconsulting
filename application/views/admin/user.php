<?php $this->load->view('admin/header',$this->data); ?>
    ...
    <div class="main-content" >
      ...
      <button class="btn btn-default" type="submit"><i class="glyphicon glyphicon-search"></i></button>
                        ...
                        <a href="<?php echo base_url(); ?>admin/user/update" class="btn btn-wide btn-warning pull-right"><i class="glyphicon glyphicon-plus-sign"></i> Add</a>
                        ...
                        if($users->num_rows())
                        {
                            ...
                            <table class="table">
                                ...
                                {
                                    $image_url = $user->image == '' ? base_url().'uploads/users/placeholder.png' : base_url().'uploads/users/'.$user->image;
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo date("jS M, Y ",strtotime($user->date_created)); ?></td>
                                        <td>
                                            <a href="<?php echo base_url(); ?>admin/user/update/?id=<?php echo $user->user_id; ?>" class="btn btn-transparent btn-xs"><i class="fa fa-pencil"></i> Edit</a>
                                            <a href="#_" id="<?php echo $user->user_id; ?>" class="btn btn-transparent btn-xs tooltips delete_button" data-toggle="modal" data-target="#DeleteModal" ><i class="fa fa-times fa fa-white"></i> Delete</a>
                                        </td>
                                        <td>
                                            <select id="<?php echo $user->user_id; ?>" class="form-control change_status_dropdown">
                                                <option value="1" <?php echo $user->is_activated == 1 ? 'selected="selected"' : ''; ?> >Active</option>
                                                <option value="0" <?php echo $user->is_activated == 0 ? 'selected="selected"' : ''; ?> >Inactive</option>
                                            </select>
...
$(document).ready(function(){
	...
	$('.change_status_dropdown').change(function(e) {
		$.ajax({
			url: "<?php echo base_url(); ?>admin/user/change_status",
			type:'POST',
			data: {'id':this.id, 'status':this.value}
		}).done(function (data){
